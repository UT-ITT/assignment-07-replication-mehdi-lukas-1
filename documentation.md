...
The replication consists of multiple Python files inside (`implementation/`) built using **OpenCV**, **NumPy**, and **SciPy**.
...
The application follows the state-machine pattern:
...
#### Phase 1 - TraceMatch Pipeline (aka. Motion Matching)
...
**TraceMatch Pipeline - Feature Detection** (`class TraceMatch - track_features`)
- Implements the motion-matching pipeline of the referenced paper TraceMatch (http://dx.doi.org/10.1145/2971648.2971714)
- Using a FAST feature detector (threshold 20, non-max suppression ON) new unique keypoints are detected each frame
- Existing keypoints are tracked using the Pyramidal Lucas-Kanade optical flow (51x51 window, 3 levels)
- Tracked motion candidates are tested using pearson correlation and RANSAC circle fitting against the moving Orbits. 

**TraceMatch Pipeline - Correlation check** (`class TraceMatch - find_matches`)  
For every motion candidate (feature with a full history):
...
3. If `max(|rx|, |ry|) ≥ 0.98` -> candidate (paper threshold).
...
5. **Connected-component labelling** seeded with 10x10 px areas around each matched feature point (paper: avoids offset errors from individual feature tracking).
6. Fit axis-aligned bounding box around the component -> initial ROI.  
   Fallback (no component): 30x30 px box around the seed point.

**CD-gain** (Eq. 8):  
`CDgain = (1 / r_fit) x screen_dim`  
...
Tracking points are placed on a grid with spacing `sqrt(wxh / 25)`, inspired by the lazy caterer's sequence mentioned in the paper. This concentrates points toward the centre of the ROI, reducing the chance of latching onto background pixels.
...
Every `RECALIB_INT = 0.5 s` (and when ROI moved > 2 px): re-run dense Farnebäck OF in a 2x search region; re-fit bounding box using connected components seeded from the current ROI centre. Record the offset between the old and new ROI centre so the cursor position does not jump (paper: *"the recalibration is unnoticeable to the user"*).

**Jitter filter** (Eq. 9)  
When frame-to-frame cursor movement `dt < dMIN = 2.5 px`:  
`NB = NMAX − ⌊dt x NMAX / dMIN⌋`  (dynamic window size, up to 10 frames)  
...
**Cursor mapping**  
`cursor_x = start_x + (roi_cx − initial_roi_cx) x CDgain`  
`cursor_y = start_y + (roi_cy − initial_roi_cy) x CDgain`  
...
Four boxes are arranged at the bottom of the screen.
Using the aquired cursor a box can be select by moving the cursor either left or right. After selecting the correct box the user can confirm the selecting by moving the cursor upwards similar to the media controls demonstrated in the paper.
...
Webcam device 0 is used. The window opens at the camera native resolution.
...
| Decision | Rationale |
|---|---|
| two orbits, single pointer | The paper's core contribution is the coupling mechanism; two orbits cleanly demonstrate it without UI overhead. |
...
| Dense OF recalibration threshold 0.5 s / 2 px | Paper values verbatim. Balances tracking accuracy against CPU cost (dense OF is ~3x more expensive per frame). |
| Values for thresholds/etc. | We decided to use the values provided by the papers when ever possible. They were reasonable defaults, which are already tested. |

#### Bootstrapping the project (TraceMatch)

##### Framework

As a starting point for the implementation, we used one of the previous OpenCV Camera task as a bootstrap framework.

At this stage we had a simple window, that rendered the content of the camera.

##### Feature Tracker

We started of by implementing the Motion Matching Pipeline describe in the TraceMatch paper. As a baseline we used the FAST Feature Detection provided by OpenCV to redetect all features/keypoints in the camera image each frame. This gave us some visual feedback/insight without having to implement the entire Feature Tracker yet.

With our keypoint detection working we decided to get started on implementing the motion candidate detection. To do that we used the displacement of the Pyramidal Lucas-Kanade optical flow included in OpenCV as described in the paper.
We considered every feature as a motion candidate (later used by the Motion Detection) if the feature displaced at least our predefined threshold.
To avoid introducing any bugs at this stage, we decided against implementing any deduplication/history window for now and kept tracking using the first detected keypoints.

Once we got that working we started to periodically redetect keypoints and implemented a history of previously tracked features as described in the paper. We also tried to remove any duplicates resulting from the periodical redetection using an Occupancy Grid. We thought that periodically redetecting features would prevent major feature drifts and an occupancy grid would be better performance-wise than doing a window check on every feature each frame.

At this stage we had a working feature detector that drew stationary features and motion candidates.

##### Widgets

With our feature detection working we started work on implementing the moving Orbit/Widget, which will be used to aquire a cursor by tracing its motion. As this was mostly about drawing a circles using OpenCV, we didn't didn't face any issues.

Since we later needed to get the trajectory (Orbit point history) for correlation, we decided to implement that now. Since the trajectory is based on time, we had to calculate the framerate/frametime, otherwise the trajectory calculation is almost impossible, if the framerate is not constant.

At this stage we had an Orbit widgets being drawn on the screen with the detected features in the background.

##### Motion Detection

Continuing with the TraceMatch pipeline, we started implementing the match detection. As a starting point we calculated the perason correlation for each motion canditate using its and the orbit's trajectory. This already worked surprisingly well and was able to find matches, but sadly had a high false positive rate, like the paper mentioned.

We used the first returned match as a gauge whether the Motion detection was even working and started work on the MedianFlowTracker in the mean time.

##### ROI/MedianFlowTracker

We implemented the ROI detection/initialisation and the MedianFlowTracker as described above in the architecture section.
The implementation was quite challanging, since it included a lot of moving parts, but we got it working like in the paper. One challange was also due to selecting the first match returned by the Motion Detection/Matcher, which obviously wasn't always the best result.

##### Interface

As last part of our first interation, we implemented a cursor and some selection targets, which are mapped to the ROI/Tracker. To get the smooth experience describe in the paper, we calculated the CD-Gain using the formular provided in the paper.

We now had our first "working" prototype of MatchPoint.


#### Refinement

After having a "working" prototype we decided to implement more features described in the paper and to follow it more closely.

First point of action was to get RANSAC working, which would discard falsely motion candiadtes after correlation. We used the formulars provided in the paper and got it working rather quickly.
But for now we still decided against trying to find the "best" match.

After getting RANSAC working, we basically had the main feature set completed. So we started work on fixing/improving the tracking/selection result.

One such fix was to scrap the idea of trying to implement a less resource intense version of the feature tracker. With the previous version tracking was iffy at best. We decided against using an occupancy grid, which caused a lot of unwanted keypoint deletions and replaced the periodical feature "redetection" by doing a simple window deduplication check each frame like the paper suggested. Even though the end result was a bit slower in theory, it still was able to run smoothly at 30 fps, so we decided it wouldn't be worth it to get the "optimized" version working no matter what.

Another fix was to implement the preprocessing described by TraceMatch, namely applying a gaussian blur to the camera image to improve tracking of moving keypoints.

Last but not least we then refactored the code to be more expandable and implemented a second input Menu/Widget, which was also featured in the paper in a slightly altered form for easier implementation.

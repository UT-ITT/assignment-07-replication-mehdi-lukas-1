...
Each webcam frame is converted to greyscale. The **FAST** (Features from Accelerated Segment Test) detector finds corner-like keypoints - image regions that move distinctively and can be reliably tracked. Settings: threshold 20, non-maximum suppression ON.

#### 3.1.3 Lucas-Kanade Optical Flow

Each detected FAST keypoint is propagated frame-to-frame using **pyramidal Lucas-Kanade** optical flow (window 51x51, 3 pyramid levels). This gives each feature a trajectory - a list of (x, y) positions over the last `HIST_LEN = 38` frames (≈ 1.25 seconds at 30 fps, covering half a revolution of the orbit).
...
2. Compute `rx = Pearson(feature_x, orbit_x)` and `ry = Pearson(feature_y, orbit_y)`.
3. If `max(|rx|, |ry|) ≥ 0.98` → potential match.

The threshold of 0.98 is taken directly from the paper and ensures accidental matches are rare.
...
**Connected-component labelling** groups the masked pixels into blobs. The algorithm is **seeded** from a 10x10 px area around each matched feature point. Only blobs connected to these seeds are retained - this avoids picking up other objects in the background that happen to have similar motion.

The bounding box of the resulting blob becomes the **initial ROI** (Region of Interest).

Fallback: if no blob is found, a 30x30 px minimum box around the feature point is used (paper §"Tracker Initialisation").
...
```
CD_gain = (1 / r_fit) x max(screen_width, screen_height)
```
...
1. Runs dense Farnebäck OF in a **2x expanded search region** around the current ROI.
...
```
cursor_x = start_x + (roi_cx_now − roi_cx_initial) x CD_gain
cursor_y = start_y + (roi_cy_now − roi_cy_initial) x CD_gain
```
...
A window titled **"MatchPoint"** opens at **1280 x 720 px**. The webcam (device 0) starts automatically.
...
| Constant | Default | Effect |
|---|---|---|
| `HIST_LEN` | `38` frames | History window. Must be > half a period x fps. Longer = more reliable but slower to trigger. |
...
| **`HIST_LEN = 38` at 30 fps** | Half a revolution of a 2.5 s orbit ≈ 1.25 s x 30 fps = 37.5 → rounded to 38. Shorter windows risk spurious correlation; longer windows delay response. |
...
| **Dense OF in 2x expanded region for recalibration** | Paper specifies searching in `2xW x 2xH` around the current ROI (§ "Tracking"). This is implemented verbatim. |
